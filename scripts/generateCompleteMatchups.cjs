/**
 * Complete Matchup Generator for MCU RPS
 *
 * Generates balanced matchups for all 18 threat level/difficulty combinations.
 * Each character beats exactly (n-1)/2 opponents in extended RPS format.
 */
...
/**
 * Generates balanced matchups using circular indexing
 */
function generateBalancedMatchups(characters) {
...
/**
 * Generates complete character registry with all matchups
 */
function generateCompleteRegistry() {
  ...
  // Track first appearance for each character
  ...
  threatLevels.forEach(threatLevel => {
    difficulties.forEach(difficulty => {
      ...
      console.log(`\nGenerating ${threatLevel}-${difficulty} (${allChars.length} characters)...`);

      // Generate balanced matchups
      const matchups = generateBalancedMatchups(allChars);
      ...
      // Validate
      allChars.forEach(charName => {
        const beats = registry[charName].beats[key];
        const expectedBeats = Math.floor((allChars.length - 1) / 2);

        if (beats.length !== expectedBeats) {
          console.error(`  ERROR: ${charName} has ${beats.length} beats, expected ${expectedBeats}`);
        }

        beats.forEach(opponent => {
          if (!allChars.includes(opponent)) {
            console.error(`  ERROR: ${charName} beats ${opponent} who is not in roster`);
          }
        });
      });

      console.log(`  ✓ Generated valid matchups for ${allChars.length} characters`);
    });
  });
...
console.log('MCU RPS - Complete Matchup Generation');
console.log('='.repeat(60));
...
console.log('\n✅ Successfully generated characters.js with complete matchups!');
