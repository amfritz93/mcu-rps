/**
 * Generate Mathematically Balanced Extended RPS Matchups
 *
 * User-provided hero foundation chains (narrative basis):
 ...
 *
 * Algorithm: Build complete circular orderings by strategically placing
 * villains and neutral characters around the hero foundation to create
 * proper transitive chains where each character beats (n-1)/2 others.
 */
...
/**
 * Build complete circular ordering by placing villains/neutral strategically
 * around the hero foundation to maintain proper extended RPS balance
 */
function buildCompleteOrdering(threatLevel, difficulty) {
  const roster = threatLevelRosters[threatLevel][difficulty];
  const heroFoundation = heroFoundations[threatLevel];

  // Get all characters categorized
  const heroes = roster.heroes.filter(h => !heroFoundation.includes(h));
  const villains = roster.villains;
  const neutral = roster.neutral;

  // Start with hero foundation
  const ordering = [...heroFoundation];

  // For balanced extended RPS, interleave additional characters
  // Strategy: Place villains between heroes to create proper spacing
  const remaining = [...villains, ...neutral, ...heroes];

  // Distribute remaining characters evenly around the circle
  const insertPositions = [];
  const step = ordering.length / remaining.length;
  for (let i = 0; i < remaining.length; i++) {
    insertPositions.push(Math.floor(step * i));
  }

  // Insert remaining characters at calculated positions
  let offset = 0;
  remaining.forEach((char, i) => {
    const pos = insertPositions[i] + offset;
    ordering.splice(pos + 1, 0, char);
    offset++;
  });
 ...
 * Each character beats the next (n-1)/2 characters in circular order
 */
function generateMatchupsFromOrdering(ordering) {
  ...
  ordering.forEach((char, i) => {
    ...
    for (let j = 1; j <= beatsCount; j++) {
      ...
      matchups[char].push(ordering[beatenIndex]);
    }
  });
...
/**
 * Verify matchups create proper transitive chains
 */
function verifyTransitiveChains(ordering, matchups) {
  const errors = [];
  const n = ordering.length;

  // Check key transitive chains
  for (let i = 0; i < Math.min(n, 10); i++) {
    const charA = ordering[i];
    const charB = ordering[(i + 1) % n];
    const charC = ordering[(i + 2) % n];
    ...
    if (aBeatsB && bBeatsC && !cBeatsA) {
      errors.push(`${charA} > ${charB} > ${charC}, but ${charC} doesn't beat ${charA}`);
    }
  }
...
/**
 * Generate complete character registry
 */
function generateCompleteRegistry() {
  ...
  console.log('MCU RPS - Mathematically Balanced Extended RPS Generation');
  console.log('='.repeat(70));
  ...
  threatLevels.forEach(threatLevel => {
    difficulties.forEach(difficulty => {
      ...
      // Build complete ordering
      const ordering = buildCompleteOrdering(threatLevel, difficulty);

      console.log(`\n${key} (${ordering.length} chars):`);
      console.log(`  ${ordering.slice(0, 5).join(' > ')}... (${ordering.length} total)`);

      // Generate matchups
      const matchups = generateMatchupsFromOrdering(ordering);

      // Verify
      const errors = verifyTransitiveChains(ordering, matchups);
      if (errors.length > 0) {
        console.log(`  ⚠️  ${errors.length} chain errors`);
      } else {
        console.log(`  ✅ Transitive chains verified`);
      }
...
// Export
const output = `/**
 * Character Registry - Mathematically Balanced Extended RPS
 *
 * Six threat levels × three difficulties = 18 complete matchup sets
 * Each character beats exactly (n-1)/2 opponents in proper transitive chains
 *
 * Hero foundations (narrative basis):
 * - Street: Luke Cage > Kate Bishop > Mockingbird
 * - Skilled: Hawkeye > Falcon > Black Widow
 * - Tech: Ant-Man > Iron Man > Shuri
 * - Enhanced: Hulk > Spider-Man > Captain America
...
console.log('\n✅ Generated characters.js with mathematically balanced matchups!');
